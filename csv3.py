...
def expand_replies():
    reply_buttons = driver.find_elements(By.XPATH, '//ytd-button-renderer[@id="more-replies"]/a')
    for button in reply_buttons:
        driver.execute_script("arguments[0].scrollIntoView();", button)
        button.click()
        time.sleep(EXPAND_REPLIES_PAUSE)
...
df.index.name = 'Index'
df.to_csv('youtube_comments_with_replies_csv3.csv', index=True)
